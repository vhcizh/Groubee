...
<%-- 				    <div class="col-lg-2 col-md-4">
                    ...
                    <div class="bg-light p-3 d-flex align-items-center do-block">
                          ...
                          <div class="ml-auto">
	                      <%-- <c:forEach var="dto" items="${list}" begin="0" end="0">  --%>
	                     <%--  <c:if test = "${dto.stateid == 'com'}"> --%>
	                      <h6 class="card-subtitle">카테고리를 선택하세요</h6>
	                       <select class="select2 form-control custom-select" name = "category" id = "category" style="width: 200px; height:36px;" required>
	                         <optgroup label="일반">
	                             <option id = "0" value="선택">선택</option>
	                             <option id = "8" value="">전체보기</option>
	                             <option id = "1" value="내부기안">내부기안</option>
	                             <option id = "2"  value="외부공문">외부공문</option>
	                         </optgroup>
	                         <optgroup label="교육">
	                             <option id = "3"  value="">교육</option>
	                             <option id = "4"  value="교육신청">교육신청</option>
	                             <option id = "5"  value="교육보고">교육보고</option>
	                         </optgroup>
	                         <optgroup label="사업진행">
	                             <option id = "6"  value="마케팅">마케팅</option>
	                             <option id = "7"  value="IT">IT</option>
	                         </optgroup>
	                      </select>
	                      <%-- </c:if> --%>
	                     <%--  </c:forEach> --%>
                    ...
                    <div class="table-responsive">
                        <table class="table email-table no-wrap table-hover v-middle">
                                ...
                                <tr class="unread">
                                    ...
                                    <td class="py-2 px-3 no-wrap text-truncate"> 
                                         <a onclick="window.open('${path}/comcheckoutDetail.fo?doc_id=${dto.doc_id}','기안문서','width=850,height=1100,scrollbars=yes');" href="${path}/approvalad.apad?stateid=com"> 
                            ...
                            <ul class="pagination justify-content-center">
                            	...
                            	<c:forEach var = "dto" items="${list}" begin = "0" end = "0">
	                            	<c:if test="${paging.startPage > 10}">
	                            		<li class="page-item"><a class="page-link" href="${path}/approvalad.apad?stateid=${dto.stateid}&category=${dto.category}&pageNum=${paging.prev}">Previous</a></li>
									</c:if>
									<!-- 페이지 -->
									<c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
										<li class="page-item"><a class="page-link" href="${path}/approvalad.apad?stateid=${dto.stateid}&category=${dto.category}&pageNum=${num}">${num}</a></li>
									</c:forEach>
									<!-- 다음 버튼 -->
									<c:if test="${paging.endPage < paging.pageCount}">
										<li class="page-item"><a class="page-link" href="${path}/approvalad.apad?stateid=${dto.stateid}&category=${dto.category}&pageNum=${paging.next}">Next</a></li>
									</c:if>
								</c:forEach>
                            </ul>
                        </nav>
        ...
        $("#category").change(function(){
            ...
            if(category == ""){
            	location.href = '${path}/approvalad.apad?stateid=com';
           		...
           		location.href = '${path}/approvalad.apad?category='+category+'&stateid=com';
            }
        })
 </script>     
