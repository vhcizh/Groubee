...
function confirmapp(doc_id) {
	if(confirm("결재하시겠습니까?")) {
		location.href = "${path}/checkoutAction.fo?doc_id="+doc_id+"&${_csrf.parameterName}=${_csrf.token}";
		
	}
}
function rejectapp(doc_id) {
	if(confirm("반려하시겠습니까?")) {
		location.href = "${path}/rejectdocAction.fo?doc_id="+doc_id+"&${_csrf.parameterName}=${_csrf.token}";
	}
}
</script>
<body>
...
&nbsp;<button type="button" onclick = "confirmapp(${dto.doc_id});"> 결재하기</button>
&nbsp;<button type="button" onclick = "rejectapp(${dto.doc_id});"> 반려하기</button> 
</tr>
</c:forEach>
<!-- Header --> 
   <colgroup> 
	...
	<tbody>
		...
		<tr>
			...
			<table style = "float:right">
					...
					<tr>
					<c:forEach var="dto" items="${list2}">
						<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; color: rgb(0, 0, 0); font-size: 10px; font-weight: normal; vertical-align: middle;  text-align: center; width: 60px; height:10px;">결재일<br>${dto.checkday}</td>
					</c:forEach>	
					</tr>
			</table>
