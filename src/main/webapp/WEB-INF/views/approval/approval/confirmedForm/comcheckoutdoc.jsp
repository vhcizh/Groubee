...
&nbsp;<button type="button" onClick = "window.open('${path}/selectReferAllmem.ap?doc_id=${dto.doc_id}','회람선','width=800,height=500,location=no,status=no,scrollbars=yes');">회람하기</button>
</c:if>
</c:forEach>
</tr>
<!-- Header --> 
	...
	<tbody>
		...
		<tr>
			...
			<table style = "float:right">
					...
					<tr>
					<c:forEach var="dto" items="${list2}">
						<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; color: rgb(0, 0, 0); font-size: 10px; font-weight: normal; vertical-align: top;  text-align: center; width: 60px; height:10px;">결재일<br>${dto.checkday}</td>
					</c:forEach>	
					</tr>
			</table>
