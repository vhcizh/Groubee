...
$(function() {
	...
	// 선택한 내용
	$(".c_natl_wrap .select2-container .select2-choice .select2-chosen",opener.document).html(c_natl_n);

	//$("#c_city",opener.document).val(c_city).trigger('change.select2');
                    ...
                    <div class="card-body">
                        <form method="post" name="submitMsgForm" enctype="multipart/form-data">
                            		...
                            		<c:forEach var="dto" items="${list}" var="dto2" items="${list2}">
									 ...
									 --%>
										<%-- <optgroup label="${dto.depart_name}"> --%>
									    	...
									    	<c:forEach var="dto_get" items="${list2}">
									    	<option value="${dto_get.getter_id}">${dto_get.getter_id} ${dto_get.depart_name} ${dto_get.name}</option>
									    	</c:forEach>
									    	<!-- getterList -->
									    	
									    	<option value="${dto.id}" selected>${dto.id} ${dto.depart_name} ${dto.name}</option>
									   <!--  </optgroup> -->
								</select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="dropzone" id="dzid">
                                <div class="fallback">
                                    <input name="files" type="file" files="${dto.files}">
                                </div>
                            </div>
                            <%-- </c:forEach> --%>
